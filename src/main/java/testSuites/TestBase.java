...
public class TestBase implements ITestListener {
	...
	public void cleanup(ITestResult result) throws IOException {
		...
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			test.fail("Error Screen Print : ", MediaEntityBuilder
					.createScreenCaptureFromPath(System.getProperty("user.dir") + "\\test-output\\Error.png").build());
		}
		/*
	...
	public void onTestFailure(ITestResult result) {
		 ...
		 * " due to " + result.getThrowable()); System.out.println("aaafffff");
		 * File f = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
		 * System.out.println("bbbfffff"); try { FileUtils.copyFile(f, new
		 * File(System.getProperty("user.dir") + "\\test-output\\Error.png"));
		 * System.out.println("ffffffff"); } catch (IOException e) {
		 * e.printStackTrace(); } try { test.fail("Error Screen Print : ",
		 * MediaEntityBuilder.createScreenCaptureFromPath(System.getProperty(
		 * "user.dir") + "\\test-output\\Error.png").build()); } catch
